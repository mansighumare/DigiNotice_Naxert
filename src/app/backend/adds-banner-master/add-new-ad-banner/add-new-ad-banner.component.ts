...
import {  AddAdBannerWebModel, AddAdBannerMobileTopModel, AddAdBannerMobileNoticeModel, AddAdBannerWebNoticeModel } from 'src/app/models/AdBanner';
...
export class AddNewAdBannerComponent implements OnInit {
 ...
 @Input() editAdBannerModel:AddAdBannerWebModel = new AddAdBannerWebModel();  
  ...
  AdBannerMobNtcModel: AddAdBannerMobileNoticeModel = new AddAdBannerMobileNoticeModel();  
  ...
  getMaxRowOrderByImageType(imageType:number){
        this.adBannerService.GetAdBannerRowByType(imageType).subscribe((maxRowOrderByType:any)=>{
            this.MaxRowOrderByType=maxRowOrderByType;
        });
  }
 
  previewWeb(filesWeb) {
    if (filesWeb.length === 0)
      return;
  ...
  onAddAdBanner(data:number){  
   ...
   if(data==1){   
    ...
    if(this.adBannerAddEdit.imagePath!==null&&this.imagePath==undefined){
      var url:string;
      url= this.adBannerAddEdit.imagePath.substr(this.adBannerAddEdit.imagePath.lastIndexOf("/")+1,this.adBannerAddEdit.imagePath.length-1);
    ...
    if(this.adBannerAddEdit.imagePath!==null&&this.imagePathMobTop==undefined){
      url= this.adBannerAddEdit.imagePath.substr(this.adBannerAddEdit.imagePath.lastIndexOf("/")+1,this.adBannerAddEdit.imagePath.length-1);
      this.adBannerAddEdit.imagePath=url;
    ...
    if(this.adBannerAddEdit.imagePath!==null&&this.imagePathMobNtc==undefined){
      url= this.adBannerAddEdit.imagePath.substr(this.adBannerAddEdit.imagePath.lastIndexOf("/")+1,this.adBannerAddEdit.imagePath.length-1);
      this.adBannerAddEdit.imagePath=url;
    ...
    if(this.adBannerAddEdit.imagePath!==null&&this.imagePathWebNtc==undefined){
      url= this.adBannerAddEdit.imagePath.substr(this.adBannerAddEdit.imagePath.lastIndexOf("/")+1,this.adBannerAddEdit.imagePath.length-1);
      this.adBannerAddEdit.imagePath=url;
  ...
  AddAdBanner(adBanner:AddAdBannerWebModel,imagePath:any){
    ...
    this.adBannerService.uploadBannerImage(imagePath[0]).subscribe((imageName:string)=>{
