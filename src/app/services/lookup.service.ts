...
import {
  NoticeInfoModel, Keyword, Country, State, City, Taluka, Village,
  UnitType, LandCategory,
  // District,
  NewsPaper
} from 'src/app/models';
...
export class LookupService {
  ...
  getTalukas(cityId: number, isActiveOnly: boolean=true) {
  ...
  getUserCityPreferenceList(stateId: number,userId:string) {
    var getCitiesApiUrl = this.appConfig.getApiPath("Lookup", "GetCity", [stateId]);
    return forkJoin([
      this.http.get(getCitiesApiUrl).pipe(),
      this.getInterestedCities(userId).pipe()
    ]);
  }
